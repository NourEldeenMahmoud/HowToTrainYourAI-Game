...
public class IntroRobotController : MonoBehaviour
{
    ...
    [Header("Message Visibility")]
    [SerializeField] private bool hideRobotPovWhileMessageOpen = true;
    [SerializeField] private string messageEHintObjectName = "E Hint Text";
    ...
    private void Awake()
    {
        ...
        EnsureMessagePanelButtonVisible();
    }
    ...
    private void OnEnable()
    {
        ...
        if (miniGame1Manager != null)
        {
            miniGame1Manager.PhaseChanged += OnMiniGame1PhaseChanged;
            ...
            {
                SyncMessagePanelButtonAvailability();
            }
        }
    }
    ...
    {
        if (miniGame1Manager != null && miniGame1Manager.IsMiniGameRunning)
            return;
    ...
    private void OnCloseButtonClicked()
    {
        ...
        MG1InstructionSequenceController instructions = MG1InstructionSequenceController.Instance ?? FindFirstObjectByType<MG1InstructionSequenceController>();
        if (instructions != null && instructions.CurrentStage == MG1InstructionSequenceController.InstructionStage.ReadTableMessage)
            instructions.SetCorrectRobotMovementStage();
    ...
    {
        if (!hideRobotPovWhileMessageOpen || robotPovCanvasRoot == null)
            return;

        if (hidden)
        {
            robotPovWasVisibleBeforeMessage = robotPovCanvasRoot.activeSelf;
            if (robotPovWasVisibleBeforeMessage)
                robotPovCanvasRoot.SetActive(false);
            return;
        }
        ...
        robotPovCanvasRoot.SetActive(true);
        EnsureInstructionTextsVisible();
        EnsureMessagePanelButtonVisible();
    }
    ...
    private void OnMiniGame1PhaseChanged(MiniGame1Manager.MiniGame1Phase phase)
    {
        SyncMessagePanelButtonAvailability();
    }
    ...
    private void EnsureMessagePanelButtonVisible()
    {
        if (miniGame1Manager != null && miniGame1Manager.IsMiniGameRunning)
            return;
        ...
        EnsureVisibleWithParents(messagePanelButton.gameObject);
        SetChildrenActiveRecursive(messagePanelButton.transform, true);
        messagePanelButton.interactable = true;

        GameObject eHint = FindChildGameObjectByName(robotPovCanvasRoot != null ? robotPovCanvasRoot.transform : null, messageEHintObjectName);
        if (eHint != null)
            EnsureVisibleWithParents(eHint);
    }

    private void SyncMessagePanelButtonAvailability()
    {
        ResolveMessagePanelButton();
        if (messagePanelButton == null)
            return;

        bool available = miniGame1Manager == null || !miniGame1Manager.IsMiniGameRunning;
        messagePanelButton.interactable = true;
        EnsureVisibleWithParents(messagePanelButton.gameObject);
        SetChildrenActiveRecursive(messagePanelButton.transform, true);
        ...
        if (!available && robotMessagesCanvas != null && robotMessagesCanvas.activeSelf)
        {
            robotMessagesCanvas.SetActive(false);
            SetRobotPovHiddenForMessage(false);
            ShowCursor(false);
    ...
    private static void SetChildrenActiveRecursive(Transform root, bool active)
    {
        if (root == null)
            return;
    ...
    private static GameObject FindChildGameObjectByName(Transform root, string objectName)
    {
        ...
        foreach (Transform child in root)
        {
            GameObject found = FindChildGameObjectByName(child, objectName);
            if (found != null)
                return found;
        }
