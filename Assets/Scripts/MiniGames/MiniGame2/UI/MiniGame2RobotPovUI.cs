...
public class MiniGame2RobotPovUI : MonoBehaviour
{
    ...
    private void AutoFindUI()
    {
        ...
        // First pass: search within the assigned povUiRoot hierarchy.
        SearchHierarchyForUI(root);

        // Second pass: if any field is still missing, walk every Canvas in the scene.
        if (AnyUIFieldMissing())
        {
            Canvas[] canvases = FindObjectsByType<Canvas>(FindObjectsSortMode.None);
            foreach (Canvas canvas in canvases)
            {
                if (canvas.transform == root) continue;
                SearchHierarchyForUI(canvas.transform);
                if (!AnyUIFieldMissing()) break;
            }
        }
    }
