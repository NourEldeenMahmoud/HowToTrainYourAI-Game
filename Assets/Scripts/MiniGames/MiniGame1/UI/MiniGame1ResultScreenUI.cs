...
{
    public event Action ApplyImprovementsClicked;
    ...
    private bool canApplyImprovements;
    ...
    public void ApplyResult(MiniGame1EvaluationResult r)
    {
        ...
        canApplyImprovements = r.tier != MiniGameTier.Fail;
        if (applyImprovementsButton != null)
            applyImprovementsButton.interactable = canApplyImprovements;

        if (!canApplyImprovements && recalibrateButton != null && EventSystem.current != null)
            EventSystem.current.SetSelectedGameObject(recalibrateButton.gameObject);
    }
    ...
    private void OnApplyImprovementsClicked()
    {
        ...
        ApplyImprovementsClicked?.Invoke();

        MG1ToMG2FlowCoordinator flowCoordinator = GetComponent<MG1ToMG2FlowCoordinator>() ?? FindFirstObjectByType<MG1ToMG2FlowCoordinator>();
        if (flowCoordinator == null)
            flowCoordinator = gameObject.AddComponent<MG1ToMG2FlowCoordinator>();

        if (flowCoordinator != null)
            flowCoordinator.BeginAfterApplyImprovements();
    }
