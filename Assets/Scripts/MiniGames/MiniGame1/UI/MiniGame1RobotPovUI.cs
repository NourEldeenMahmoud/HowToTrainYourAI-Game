...
public class MiniGame1RobotPovUI : MonoBehaviour
{
    ...
    [Header("UI Texts")]
    [Tooltip("Optional challenge label text. Do not assign the main instruction text here unless updateObjectiveTextFromMiniGamePhases is intentionally enabled.")]
    ...
    [Tooltip("Enable only if challengeNameText is a separate challenge label, not the main instruction/objective text.")]
    [SerializeField] private bool updateObjectiveTextFromMiniGamePhases;
    ...
    private MiniGame1Manager.MiniGame1Phase? lastPhase;
    ...
    {
        if (lastPhase.HasValue && phase == lastPhase.Value) return;
        lastPhase = phase;

        MiniGame1PhaseUiText phaseText = miniGame1Manager != null
            ? miniGame1Manager.GetPhaseUiText(phase)
            : default;

        ApplyChallengeText(phaseText.challengeText, phaseText.promptText);

        if (phase == MiniGame1Manager.MiniGame1Phase.FreeMoveInitial)
        {
            SetLog("");  // Clear log at mini-game start
        }
        else if (phase == MiniGame1Manager.MiniGame1Phase.None)
        {
            SetLog(string.Empty);
        }
    }

    private void ApplyChallengeText(string challenge, string prompt)
    {
    ...
    {
        if (!updateObjectiveTextFromMiniGamePhases)
            return;
