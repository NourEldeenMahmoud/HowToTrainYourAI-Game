...
public class RobotStatsSO : ScriptableObject
{
    ...
    [Header("Post-MiniGame Fault Probabilities (0..1)")]
    [Tooltip("Chance for a yaw drift fault to trigger at each post-MG1 drift fault check.")]
    [Range(0f, 1f)] public float driftErrorRate = 0.50f;
    [Tooltip("Chance for a camera pitch fault to trigger at each post-MG1 camera fault check.")]
    [Range(0f, 1f)] public float cameraErrorRate = 0.50f;
    [Tooltip("Chance for a sprint cancel/block fault to trigger at each post-MG1 speed fault check.")]
